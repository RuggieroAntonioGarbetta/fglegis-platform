...
def register_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Registrazione completata. Attendi l’attivazione.")
...
def get_pratiche_per_utente(user):
    ...
    if profilo.ruolo == 'admin':
        ...
        store_manager_users = ProfiloUtente.objects.filter(
            titolare=user, ruolo='store_manager').values_list('user',
                                                              flat=True)
        agenti_users = ProfiloUtente.objects.filter(
            titolare__in=store_manager_users,
            ruolo='agente').values_list('user', flat=True)
        ...
        agenti_users = ProfiloUtente.objects.filter(
            titolare=user, ruolo='agente').values_list('user', flat=True)
...
def area_personale_view(request):
    ...
    if profilo.ruolo == 'titolare_agenzia':
        ...
        if sito_web_personalizzato:
            colore_primario, colore_accento = estrai_colori_da_sito(
                sito_web_personalizzato)

    elif profilo.ruolo in ['store_manager', 'agente']:
        if profilo.titolare_id:
            titolare = ProfiloUtente.objects.filter(
                user=profilo.titolare_id, ruolo='titolare_agenzia').first()
            if titolare:
    ...
    return render(
        request, 'real_estate/area_personale.html', {
            'pratiche_info':
            pratiche_info,
            'profilo':
            profilo,
            'ruoli_per_creare_pratica':
            ['admin', 'titolare_agenzia', 'store_manager', 'agente'],
            'today':
            timezone.now().date(),
            'logo_personalizzato':
            logo_personalizzato,
...
from .forms import CreaPraticaForm
from .models import PraticaImmobiliare
from .utils import invia_email_api_brevo  # ✅ Import corretto e fuori dalle funzioni

from django.contrib.auth.decorators import login_required
from django.shortcuts import render, redirect
from django.contrib import messages
from .forms import CreaPraticaForm
from .models import PraticaImmobiliare, Documento, LogAttivitaPratica
from .utils import invia_email_api_brevo
...
def crea_pratica_view(request):
    if request.method == 'POST':
        form = CreaPraticaForm(request.POST, request.FILES, user=request.user)
        if form.is_valid():
            ...
            # Invia email al notaio se assegnato
            if pratica.notaio and pratica.notaio.user.email:
                invia_email_api_brevo(
                    to_email=pratica.notaio.user.email,
                    subject=
                    '📬 Sei stato assegnato a una nuova pratica - FG LEGIS',
                    content=f"""
                        <p>Ciao {pratica.notaio.user.username},</p>
                ...
                invia_email_api_brevo(
                    ...
                    content=
                    f"Notaio {pratica.notaio.user.username} assegnato alla pratica {pratica.nome_azienda}."
                )

            # Invia email al geometra se assegnato
            if pratica.geometra and pratica.geometra.user.email:
                invia_email_api_brevo(
                    to_email=pratica.geometra.user.email,
                    subject=
                    '📬 Sei stato assegnato a una nuova pratica - FG LEGIS',
                    content=f"""
                        <p>Ciao {pratica.geometra.user.username},</p>
                ...
                invia_email_api_brevo(
                    ...
                    content=
                    f"Geometra {pratica.geometra.user.username} assegnato alla pratica {pratica.nome_azienda}."
                )

            # Documenti predefiniti in base al tipo di pratica
            documenti_predefiniti = {
                ...
                'catastale':
                ['Estratto mappa', 'Scheda catastale', 'Richiesta ufficiale'],
            }
...
def dettaglio_pratica_view(request, pratica_id):
    ...
    acquirente_form = InfoAcquirenteForm(request.POST or None,
                                         instance=pratica)
    documento_form = CaricaDocumentoForm(request.POST or None, request.FILES
                                         or None)
    nota_form = NotaPraticaForm()
    assegna_form = AssegnaProfessionistiForm()

    documenti_predefiniti = [
        "Antiriciclaggio", "Proposta di acquisto", "Preliminare",
        "Incarico di mediazione", "Incarico FG Consulting",
        "Delega accesso atti", "Delega scarico planimetrie"
    ]
    ...
    if request.method == 'POST':
        if 'salva_venditore' in request.POST and venditore_form.is_valid():
            ...
            return redirect('real_estate:dettaglio_pratica',
        ...
        if 'upload_documento' in request.POST and documento_form.is_valid():
            for file in request.FILES.getlist('file'):
                nome = file.name
                doc, _ = Documento.objects.get_or_create(pratica=pratica,
                                                         nome=nome)
                doc.file = file
            ...
            return redirect('real_estate:dettaglio_pratica',
                            pratica_id=pratica.id)
        if 'upload_singolo_documento' in request.POST:
                documento_id = request.POST.get("documento_id")
                file = request.FILES.get("file")
                if documento_id and file:
                    documento = Documento.objects.get(id=documento_id)
                    documento.file = file
                    documento.caricato_il = timezone.now()
                    documento.save()

                    LogAttivitaPratica.objects.create(
                        pratica=pratica,
                        utente=request.user,
                        azione=f"Caricato documento singolo: {documento.nome}"
                    )
        ...
        if 'rispondi_nota' in request.POST:
            ...
            if risposta_a and testo:
                NotaPratica.objects.create(pratica=pratica,
                                           autore=request.user,
                                           testo=testo,
                ...
                return redirect('real_estate:dettaglio_pratica',
                                pratica_id=pratica.id)
    if 'assegna_professionisti' in request.POST:
        assegna_form = AssegnaProfessionistiForm(request.POST)
        if assegna_form.is_valid():
            notaio = assegna_form.cleaned_data['notaio']
            geometra = assegna_form.cleaned_data['geometra']

            pratica.notaio = notaio.user if notaio else None
            pratica.geometra = geometra.user if geometra else None
            pratica.save()

            destinatari = []
            if notaio:
                destinatari.append(notaio.user.email)
            if geometra:
                destinatari.append(geometra.user.email)
            destinatari.append("legal.fgconsulting@gmail.com")

            from .utils import invia_email_api_brevo
            for email in destinatari:
                invia_email_api_brevo(
                    to_email=email,
                    subject="📌 Nuova Assegnazione Pratica FG LEGIS",
                    content=
                    f"Ti è stata assegnata una nuova pratica: {pratica.nome_azienda}."
                )

            messages.success(request,
                             "Notaio e/o Geometra assegnati con successo.")
            return redirect('real_estate:dettaglio_pratica',
                            pratica_id=pratica.id)
    ...
    return render(
        request, 'real_estate/dettaglio_pratica.html', {
            'pratica': pratica,
            'documenti': documenti,
            'caricati': caricati,
            'mancanti': mancanti,
...
def area_notaio_view(request):
    ...
    if request.method == 'POST':
        ...
        if pratica_id and nome_documento:
            pratica = get_object_or_404(PraticaImmobiliare,
                                        id=pratica_id,
                                        notaio=request.user)
            ...
            store_manager = ProfiloUtente.objects.filter(
                user__in=[autore],
                ruolo='agente').first().titolare if autore else None

            titolare = None
            if store_manager:
                titolare = ProfiloUtente.objects.filter(
                    user=store_manager, ruolo='store_manager').first().titolare
            ...
            invia_email_api_brevo(
                ...
                content=
                f"{profilo.ruolo} {request.user.username} ha caricato un documento nella pratica {pratica.nome_azienda}."
            )
            ...
            LogAttivitaPratica.objects.create(
                ...
                utente=request.user,
                azione=
                f"Notaio ha richiesto un nuovo documento: “{nome_documento}”.")
            messages.success(request,
                             "Documento richiesto aggiunto con successo.")
            return redirect('real_estate:area_notaio')

    pratiche = PraticaImmobiliare.objects.filter(notaio=request.user)
    return render(request, 'real_estate/area_notaio.html',
                  {'pratiche': pratiche})
...
def area_geometra_view(request):
    ...
    if request.method == 'POST':
        ...
        if pratica_id and nome_documento and file:
            pratica = get_object_or_404(PraticaImmobiliare,
                                        id=pratica_id,
                                        geometra=request.user)
            Documento.objects.create(pratica=pratica,
                                     nome=nome_documento,
                                     file=file,
            ...
            invia_email_api_brevo(
                ...
                subject='📄 Documento caricato dal geometra',
                content=
                f"Geometra {request.user.username} ha caricato un documento nella pratica {pratica.nome_azienda}."
            )
            ...
            LogAttivitaPratica.objects.create(
                ...
                utente=request.user,
                azione=
                f"Geometra ha caricato un documento tecnico: '{nome_documento}'."
            )
            messages.success(request,
                             "Documento tecnico caricato con successo.")
            return redirect('real_estate:area_geometra')

    pratiche = PraticaImmobiliare.objects.filter(geometra=request.user)
    return render(request, 'real_estate/area_geometra.html',
                  {'pratiche': pratiche})
...
def crea_annuncio_ai_view(request, pratica_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Annuncio AI creato. L'elaborazione inizierà a breve.")
            return redirect('real_estate:area_personale')
    else:
